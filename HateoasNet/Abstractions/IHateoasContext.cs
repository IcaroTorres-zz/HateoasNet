...
namespace HateoasNet.Abstractions
{
	...
	public interface IHateoasContext
	{
		...
		/// <returns>
		/// <seealso cref="System.bool"/> if exists a <see cref="IHateoasResource{T}"/>
		/// for T being <paramref name="type"/>.
		/// </returns>
		bool HasSet(Type type);
		...
		/// <summary>
		/// Applies all configurations of HATEOAS resources found in classes implementing <see cref="IHateoasResourceConfiguration{T}"/>
		...
		/// <returns>Current <see cref="IHateoasContext"/> instance.</returns>
		IHateoasContext ApplyConfigurationsFromAssembly(Assembly assembly);
	}
}